...
import Lighting_Group from "~/assets/scripts/objects/Lighting_Group";
...
export default {
  data() {
    return {
      ...
      tp: new TablePaging({
        ...
        data: API.Lighting_Group_GetList,
      ...
      form: new DefaultForm({
        ...
        ShowForm: (title, isAdd, obj) => {
          ...
          this.form.obj = new Lighting_Group(obj);
  ...
  methods: {
    ...
    Add() {
      this.form.ShowForm("Thêm nhóm chiếu sáng", true);
    },
    Edit(row) {
      this.form.ShowForm("Sửa nhóm chiếu sáng", false, row);
    },
    Delete(row) {
      ShowConfirm({
      ...
      this.$refs.form.getValidate().then((re) => {
        if (!re) {
          ...
          let api = this.form.obj.Id
            ? API.Lighting_Group_Edit
            : API.Lighting_Group_Add;
          GetDataAPI({
            method: "post",
            url: api,
